 ...
 * Classe dediee a la piece espion
 *
 */

public class Spy extends Piece {
	
	private final static int[] CANDIDATE_MOVE_COORDINATES= {-10,-1,1,10}; // decalages de la position de la pièce, liste des mouvements autorises
                                                                          // -1 correspond à aller vers la gauche
	public Spy(Team pieceTeam, int piecePosition) {                       //  1 correspond à aller vers la droite
		super(PieceType.SPY, piecePosition, pieceTeam);                   // -10 correspond à aller vers l'arrière
		this.pieceRank=0;                                                 //  10 correspond à aller vers l'avant
	} 
		...
		for(final int currentCandidateOffset : CANDIDATE_MOVE_COORDINATES) {
			...
			if(BoardUtils.isValidSquareCoordinate(candidateDestinationCoordinate)) {
				...
				if(isFirstColumnExclusion(this.piecePosition, currentCandidateOffset) ||
					...
					continue; // cette condition permet d'exclure les mouvements realises dans la 1ère et dans la dernière collone(dans ce cas le decalage est incoherent)
				}             
	...
	@Override
	public Spy movePiece(Move move) {
		
		return new Spy(move.getMovedPiece().getPieceTeam(), move.getDestinationCoordinate());
	}
}